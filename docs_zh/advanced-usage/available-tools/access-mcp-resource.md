---
description: 了解 access_mcp_resource 工具如何从 Model Context Protocol 服务器检索数据，为 Roo Code 任务提供额外上下文。
keywords:
  - access_mcp_resource
  - MCP
...
`access_mcp_resource` 工具从连接的 Model Context Protocol (MCP) 服务器公开的资源中检索数据。它允许 Roo 访问文件、API 响应、文档或系统信息，为任务提供额外的上下文。
...
此工具连接到 MCP 服务器并从其公开的资源中获取数据。与执行操作的 `use_mcp_tool` 不同，此工具专门检索作为任务上下文的信息。
...
- 当 Roo 需要访问来自专业 MCP 服务器的领域特定数据时
...
3. **资源请求**：
   ...
   - 通过 MCP 中央向服务器发送 `resources/read` 请求
...
- 在协助 API 开发时，Roo 从 MCP 资源检索端点规范以确保正确实现。
- 在处理数据可视化时，Roo 访问来自已连接 MCP 服务器的当前数据样本。
- 在专业领域工作时，Roo 检索技术文档以提供准确指导。
- 在生成特定行业代码时，Roo 引用文档资源中的合规性要求。
