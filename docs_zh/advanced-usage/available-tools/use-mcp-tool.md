---
description: 了解 use_mcp_tool 如何集成外部 MCP 服务器，为 Roo Code 扩展专用工具、领域特定功能和外部服务。
keywords:
  - use_mcp_tool
  - MCP 工具
...
`use_mcp_tool` 工具支持与已连接的 Model Context Protocol (MCP) 服务器提供的外部工具进行交互。它通过标准化协议扩展 Roo 的能力，提供领域特定功能。
...
此工具允许 Roo 访问外部 MCP 服务器提供的专用功能。每个 MCP 服务器可提供多种工具，具有独特能力，从而扩展 Roo 超越其内置功能的范围。系统使用架构验证参数，管理服务器连接，并处理各种响应内容类型（文本、图像、资源）。
...
- 在客户端和服务器端使用 Zod 架构验证验证参数
- 处理多种响应内容类型：文本、图像和资源引用
- 管理服务器生命周期，在服务器代码更改时自动重启
- 提供“始终允许”机制，绕过受信任工具的审批
- 与配套工具 `access_mcp_resource` 配合使用，用于资源检索
- 维护适当的错误跟踪和处理机制，应对失败操作
- 支持可配置超时（1-3600 秒，默认：60 秒）
...
- **全局配置**：通过 VS Code 中的 Roo Code 扩展设置管理。这些设置适用于所有项目，除非被覆盖。
- **项目级配置**：在项目根目录的 `.roo/mcp.json` 文件中定义。
 - 允许项目特定的服务器设置。
 - 项目级服务器优先于同名的全局服务器。
 - 由于 `.roo/mcp.json` 可提交到版本控制，便于与团队共享配置。
...
2. **执行和通信**：
   - 系统选择适当的传输机制：
     ...
     - `SSEClientTransport`：通过 Server-Sent Events 与 HTTP 服务器通信
     - `StreamableHTTPClientTransport`：通过 Streamable HTTP Events 与 HTTP 服务器通信
...
- 用户必须在执行前批准工具使用（默认）
- 可将特定工具标记为自动批准，加入“始终允许”列表
- 使用 Zod 架构验证服务器配置以确保完整性
- 可配置超时防止操作挂起（1-3600 秒）
- 可通过 UI 启用或禁用服务器连接
