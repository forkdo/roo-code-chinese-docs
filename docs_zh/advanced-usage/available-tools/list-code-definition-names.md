---
description: 通过 list_code_definition_names 工具列出类、函数和接口，获取代码库的结构化概览。
keywords:
...
`list_code_definition_names` 工具通过列出指定目录顶层源文件中的代码定义，为您的代码库提供结构化概览。它通过显示行号和定义片段，帮助 Roo 理解代码架构。
...
该工具扫描指定目录顶层的源文件，并提取类、函数、接口等代码定义。它会显示每个定义的行号和实际代码，为您提供快速映射代码库重要组件的方法。
...
- 探索不熟悉的代码库时，Roo 在深入实现细节之前先映射重要代码结构。
- 添加新功能时，Roo 识别现有模式和相关代码定义以保持一致性。
- 排查 bug 时，Roo 映射代码库结构以定位问题的潜在来源。
- 规划架构变更时，Roo 识别所有受影响的跨文件组件。
