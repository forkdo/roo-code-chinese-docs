---
description: 在 Roo Code 中使用模型上下文协议 (MCP) 的完整指南。了解如何配置 MCP 服务器、管理工具以及扩展 Roo Code 的功能。
keywords:
  - MCP
...
MCP（模型上下文协议）服务器充当桥梁，让 Roo Code 能够访问更广泛的**工具**和外部服务，例如数据库、API 或自定义脚本。它使用标准的通信方法，使 Roo 能够利用这些外部功能。
...
**前提条件：** 确保在 MCP 设置面板中勾选了 **[启用 MCP 服务器创建](#enabling-or-disabling-mcp-server-creation)** 设置。如果此选项被禁用，Roo 将没有构建服务器所需的必要指令。
...
2.  **Roo 的处理流程（简化）：** 一旦您提出请求（且设置已启用），Roo 将：
    ...
    *   **处理密钥：** 如果工具需要 API 密钥或其他凭据，Roo 将使用 [`ask_followup_question`](/advanced-usage/available-tools/ask-followup-question) 工具向您询问它们，以确保它们被安全地配置为服务器的环境变量。
...
此功能允许您通过让 Roo 直接根据您的请求构建您需要的特定集成来定制其功能。要深入了解内部机制，请参阅[工具调用机制](/advanced-usage/available-tools/tool-use-overview#tool-calling-mechanism)。
...
* **询问 Roo：** 您可以要求 Roo Code 帮助您查找甚至创建 MCP 服务器（当“[启用 MCP 服务器创建](#enabling-or-disabling-mcp-server-creation)”启用时）
...
*   **使用自定义指令进行增强：** 除了嵌入在 MCP 服务器中的描述之外，你还可以通过提供[自定义指令](/features/custom-instructions)来进一步指导 Roo 对特定 MCP 工具的使用。这允许你定义首选方法、概述涉及多个工具的复杂工作流程，或指定何时应优先考虑或避免使用某个特定的 MCP 工具。

#### 3. 工具调用
如果 Roo 在工具描述的引导下识别出合适的工具，它会提出使用建议。然后你批准此操作（除非为受信任的工具配置了[自动批准](#auto-approve-tools)）。
